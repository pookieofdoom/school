...
void send_ack(Connection *client, uint32_t recv_seq_num, uint8_t flag, uint32_t *sever_seq);
...
void send_ack(Connection *client, uint32_t recv_seq_num, uint8_t flag, uint32_t *server_seq)
{
   uint8_t packet[MAX_LEN];
   uint8_t data[MAX_LEN];
   if (flag == RR || flag == END_OF_FILE)
      recv_seq_num++;
   memcpy(&data[0], &(recv_seq_num), sizeof(uint32_t));
   send_buf(data, sizeof(uint32_t), client, flag, *server_seq, packet);
   (*server_seq)++;
}
