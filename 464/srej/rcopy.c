...
uint32_t get_ack(uint8_t *recv_ack);
void update_window(Window_Buffer *window_buf, int32_t window_size, int32_t *bottom_window, int32_t *upper_edge, uint8_t recv_ack);
...
uint32_t get_ack(uint8_t *recv_ack)
{
   ...
   uint32_t recv_seq = 0;
    //receive RR or SREJ
   if (recv_buf(recv_ack, 1000, server.sk_num, &server, &recv_flag, &recv_seq) != CRC_ERROR)
   {
